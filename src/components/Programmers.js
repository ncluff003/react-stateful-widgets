...
export default function Programmers() {
  ...
  const [awesomeProgrammer, setAwesomeProgrammer] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers[awesomeProgrammer - 1].name;
  };

  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: awesomeProgrammer === null ? "royalblue" : "gold", // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className="programmers">
        {
          ...
          programmers.map((dev) => (
            <div className="programmer" key={dev.id}>
              ...
              <button
                onClick={() => {
                  setAwesomeProgrammer(dev.id); /* in here set the featured id to be dev.id */
                }}
              >
                Feature
              </button>
      ...
      <div id="featured" style={style}>
        {
          ...
          awesomeProgrammer ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : "Pick an awesome programmer"
        }
      </div>
    </div>
  );
